...
public static partial class ExtensionMethods
{
    ...
    /// <summary>
    /// Inverts the red and blue channels of an ARGB color.
    /// </summary>
    public static uint InvertRedBlue(this uint argb)
    {
        // Extract the red and blue channels
        byte red = (byte)((argb >> 16) & 0xFF); // Red is in bits 16-23
        byte blue = (byte)(argb & 0xFF);        // Blue is in bits 0-7

        // Clear the existing red and blue channels, then set them with swapped values
        return (argb & 0xFF00FF00) | ((uint)blue << 16) | red;
    }
    ...
    /// </summary>
    public static System.Numerics.Vector4 SetAlphaARGB(this System.Numerics.Vector4 vec, float alpha)
    {
        vec.W = alpha;
        return vec;
    }
    ...
    /// </summary>
    /// <remarks>Color channels are normalized.</remarks>
    public static System.Numerics.Vector3 RGBToVector3(this uint argb)
    {
        // Extract the color components from the argb number
        float r = ((argb >> 16) & 0xFF) / 255f; // Red component (x)
        float g = ((argb >> 8) & 0xFF) / 255f;  // Green component (y)
        float b = (argb & 0xFF) / 255f;         // Blue component (z)
    ...
    /// </remarks>
    public static System.Numerics.Vector4 RGBToVector4(this uint argb)
    {
        // Extract the individual color components
        float r = (argb >> 16) & 0xFF;
        float g = (argb >> 8) & 0xFF;
